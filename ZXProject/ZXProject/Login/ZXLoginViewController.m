...
- (void)setupView {
    ...
    passwordView.inputTextField.text = @"";
    ...
    registerBtn.titleLabel.font = Font(11);
    ...
    forgetPwBtn.titleLabel.font = Font(11);
...
- (void)loginAction {
    ...
    NSDictionary *dic = @{@"userMobile" : phoneView.inputTextField.text,
                          @"userPwd" : passwordView.inputTextField.text,
...
- (void)handleData:(id)data byRequestId:(NSInteger)requestId {
    ...
    if (requestId == ZXInterfaceLogin) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            loginModel = [[ZXLoginModel alloc] initWithDictionary:data error:nil];
            
            NSString *idString = [NSString stringWithFormat:@"%@", [ZXLoginModel sharedZXLoginModel].loginUser.userId];
            [self sendRequestId:ZXInterfaceUserInfo rMethod:GETMethod params:@{@"userId" : idString}];
            
        }else {
            [ZXTools makeTask:data[@"message"]];
        }
        
    }else if (requestId == ZXInterfaceWeChatLogin) {
        ...
        if ([bindString isKindOfClass:[NSString class]] && [bindString isEqualToString:@"false"]) {
            ...
            if ([data[@"data"][@"user"] isKindOfClass:[NSDictionary class]]) {
                ...
                if ([self didPhoneTurnToV:loginModel.loginUser.userMobile]) {
                    ...
                    NSLog(@"%@", [ZXLoginModel sharedZXLoginModel].loginUser.userId);
        ...
        ZXLoginModel *loginModel = [ZXLoginModel sharedZXLoginModel];
        loginModel.loginUser = [[LoginUserModel alloc] initWithDictionary:data[@"loginUserDesc"] error:nil];
        
        NSDictionary *infoDic = [loginModel.loginUser modelToJSONObject];
        [[NSUserDefaults standardUserDefaults] setObject:infoDic forKey:kUserLoginInfo];
...
- (void)backAct {
    ...
    CustomTabBarController* tabbar=[ZXLoginModel getTabbar];
