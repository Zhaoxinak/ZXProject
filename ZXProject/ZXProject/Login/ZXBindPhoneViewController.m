...
- (void)setupView {
    ...
    NSString *mobilPhone = [NSString stringWithFormat:@"%@",loginModel.loginUser.userMobile];
...
- (void)confirmAction {
    ...
    ZXLoginModel *loginModel = [ZXLoginModel sharedZXLoginModel];
    NSString *userID = [NSString stringWithFormat:@"%@",loginModel.loginUser.userId];
...
- (void)handleWeChatBindData:(id)data {
    ...
    if ([code isEqualToString:@"200"]) {
        [[NSUserDefaults standardUserDefaults] setObject:@{@"phone":phoneView.inputTextField.text,
                                                           @"userid":[ZXLoginModel sharedZXLoginModel].loginUser.userId
                                                           } forKey:@"userPhone"];
        ZXLoginModel *loginModel = [ZXLoginModel sharedZXLoginModel];
        ...
        if ([advanceString isEqualToString:@"false"]) {
            ...
            NSDictionary *param = @{
                                    ...
                                    @"picture"  : loginModel.loginUser.headimg ? loginModel.loginUser.headimg : @"",
                                    @"nikeName" : loginModel.loginUser.nickname ? loginModel.loginUser.nickname : @"",
                                    @"unionid" : loginModel.loginUser.unionid ? loginModel.loginUser.unionid : @""};
...
- (void)handleViMobileData:(id)data {
    ...
    if ([message isEqualToString:@"验证成功"]) {
        ...
        [ZXTools makeTask:@"原手机号验证不通过,请重试"];
        return;
    }
